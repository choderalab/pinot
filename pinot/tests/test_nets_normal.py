...
def test_nets_normal_shallow():
    ...
    theta = torch.distributions.normal.Normal(
            loc=10 * torch.ones(size=(1, 2)),
            scale=10 * torch.ones(size=(1, 2))).sample()
    ...
    y = torch.distributions.normal.Normal(
        loc=torch.zeros(size=(1024, )),
        scale=torch.ones(size=(1024, ))).sample()
    ...
    npt.assert_almost_equal(
        theta[0, 0].detach().numpy(),
        torch.mean(y).detach().numpy(),
        decimal=3)
